...
public class JoystickState implements ControllerState {
    ...
    public static JoystickState fromJoystick(JoystickController<?> joystick, int joystickId) {
        ...
        if (DebugProperties.PRINT_JOY_STATE) {
            CUtil.LOGGER.info("Printing joystick state for controller {}", joystick);
            CUtil.LOGGER.info(Arrays.stream(axes).map(axis -> axis.name().getString() + ": " + axis.getAxis(data)).toList().toString());
            CUtil.LOGGER.info(Arrays.stream(mapping.buttons()).map(button -> button.name().getString() + ": " + button.isPressed(data)).toList().toString());
            CUtil.LOGGER.info(Arrays.stream(mapping.hats()).map(hat -> hat.name().getString() + ": " + hat.getHatState(data)).toList().toString());
        }
