 ...
 */
public class SignalSynthesizerStage implements PacedEventStage<InputEvent, Signal> {
    ...
    public void onEvent(InputEvent event, EventSink<? super Signal> downstream) {
        switch (event) {
            case InputEvent.ButtonChanged(long timeNanos, ResourceLocation btn, boolean state) -> {
                if (state) {
    ...
    @Override
    public EventSubscription attachClock(Clock clock, EventSink<? super Signal> downstream) {
        return clock.subscribe(tick -> {
            this.buttons.values().forEach(t -> t.onTick(tick.timestamp(), downstream));

            downstream.accept(new Signal.Tick(tick.timestamp()));
        });
    }
    ...
    private final class AxisTracker {
        private static final float HYSTERESIS_THRESHOLD = 0.001f;
        ...
        private void onMove(long time, float value, EventSink<? super Signal> downstream) {
            ...
            if (Math.abs(delta) >= HYSTERESIS_THRESHOLD) {
                lastValue = pendingValue;
                downstream.accept(new Signal.AxisMoved(time, id, value, delta));
            }
            pendingValue = value;
        }
    }
