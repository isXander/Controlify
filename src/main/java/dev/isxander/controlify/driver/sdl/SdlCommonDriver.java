...
public abstract class SdlCommonDriver<SDL_Controller> implements Driver {
    ...
    protected final EventSource<SdlControllerEvent> eventSource;
    ...
    public SdlCommonDriver(
            SDL_Controller ptrController,
            SDL_JoystickID jid,
            ControllerType type,
            EventSource<SdlControllerEvent> eventSource,
        ...
        this.eventSource.filter(SdlControllerEvent.Battery.class)
                .subscribe(this::onBatteryEvent);
    ...
    protected void onBatteryEvent(SdlControllerEvent.Battery event) {
        PowerState level = switch (event.state()) {
            case ERROR, UNKNOWN -> new PowerState.Unknown();
            case ON_BATTERY -> new PowerState.Depleting(event.percent());
            case NO_BATTERY -> new PowerState.WiredOnly();
            case CHARGING -> new PowerState.Charging(event.percent());
            case CHARGED -> new PowerState.Full();
        };
    ...
    private void updateDualSense() {
        ...
        boolean dirty = false;
        if (this.dualSenseComponent.consumeLeftTriggerDirty()) {
            dirty = true;
        ...
        if (this.dualSenseComponent.consumeRightTriggerDirty()) {
            dirty = true;
